...
class ScenarioUpgrader(ScenarioTransformer):
    ...
    def move_console_vuart(self, xsd_element_node, xml_parent_node, new_nodes):
        ...
        if legacy_vuart is not None and legacy_vuart.find("type").text == "VUART_LEGACY_PIO":
    ...
    def move_guest_flag(self, guest_flag, xsd_element_node, xml_parent_node, new_nodes):
        new_node = etree.Element(xsd_element_node.get("name"))
        old_data_nodes = self.get_from_old_data(xml_parent_node, f".//{new_node.tag}")
        if old_data_nodes:
            new_node.text = old_data_nodes[0].text
        else:
    ...
    def move_data_by_xpath(self, xpath, xsd_element_node, xml_parent_node, new_nodes, scenario_xml_only = False, launch_xml_only = False):
        ...
        if self.complex_type_of_element(xsd_element_node) is None:
            ...
            # For each complex type containing multiple configuration items, this method can only create one single
            # node, as there is no way for the default data movers to migrate multiple pieces of data of the same type
            # to the new XML.
            new_node = etree.Element(element_tag)
            new_nodes.append(new_node)
            return True
    ...
    data_movers = {
        ...
        "pcpu_id": partialmethod(move_data_from_either_xml, "cpu_affinity/pcpu_id", "cpu_affinity/pcpu_id"),
