...
class ScenarioTransformer:
    ...
    def type_of_element(self, type_tag, xsd_element_node, xml_node):
        ...
        if xml_node is not None:
            for alternative in xsd_element_node.findall("xs:alternative", namespaces=self.xpath_ns):
                if xml_node.xpath(alternative.get("test")):
    ...
    def transform_sequence(self, xsd_sequence_node, xml_node):
        ...
        for xsd_element_node in xsd_sequence_node.findall("xs:element", namespaces=self.xpath_ns):
            ...
            if len(children) == 0 or children[0].tag != element_name:
                if self._visit_optional_node or element_min_occurs != "0":
                    anchor_node = children[0] if len(children) > 0 else None
                    self.add_and_transform_missing_node(xsd_element_node, xml_node, xml_anchor_node=anchor_node)
            else:
                while len(children) > 0 and children[0].tag == element_name:
                    self.transform_node(xsd_element_node, children.pop(0))
    ...
    def add_and_transform_missing_node(self, xsd_element_node, xml_parent_node, xml_anchor_node=None):
        for new_node in self.add_missing_nodes(xsd_element_node, xml_parent_node, xml_anchor_node):
