...
def gen_root_pci_bus(name, prt_packages):
    resources = []
...
def add_or_replace_object(template_tree, new_tree):
    name = new_tree.NameString.value
    objects_tree = template_tree.TermList.children[0].TermList
...
def discard_external_objects(tree):
    objects_tree = tree.TermList.children[0].TermList
...
def gen_dsdt(board_etree, scenario_etree, allocation_etree, vm_id, dest_path):
        ...
        if m:
            ...
            if device_node is not None and alloc_node is not None:
                ...
                if template is not None:
                    tree = parse_tree("DefScope", bytes.fromhex(template.text))
                    vaddr = (vdev << 16) | vfunc
                ...
                for peer_device_node in collect_dependent_devices(board_etree, device_node):
                    ...
                    if template is not None:
                        tree = parse_tree("DefScope", bytes.fromhex(template.text))
                        objects.append(discard_external_objects(tree))
    ...
    if root_pci_bus:
        ...
        if acpi_object is not None:
            ...
            scope = Context.parent(path)
            name = path[-4:]
            tree = builder.DefScope(
                builder.PkgLength(),
                scope,
                gen_root_pci_bus(name, prt_packages))
    ...
    s5_object = builder.DefName(
        ...
        builder.DefPackage(
            ...
            builder.PackageElementList(
                ...
                builder.build_value(0))))
    objects.append(s5_object)

    amlcode = builder.AMLCode(header, *objects)
